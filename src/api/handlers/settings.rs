...
#[derive(Deserialize)]
pub struct TestGoogleRequest {
    pub service_account_json: String,
    pub domain: String,
}

#[derive(Deserialize)]
pub struct TestBingWebmasterRequest {
    pub bing_webmaster_api_key: String,
    pub domain: String,
}

#[derive(Deserialize)]
pub struct TestIndexNowRequest {
...
pub async fn handle_test_google(
    ...
    Json(request): Json<TestGoogleRequest>,
) -> (StatusCode, Json<serde_json::Value>) {
    match state
        .url_service
        .test_google_credentials(&request.service_account_json, &request.domain)
        .await
    {
...
pub async fn handle_test_bing_webmaster(
    State(state): State<AppState>,
    Json(request): Json<TestBingWebmasterRequest>,
) -> (StatusCode, Json<serde_json::Value>) {
    match state
        .url_service
        .test_bing_webmaster_key(&request.bing_webmaster_api_key, &request.domain)
        .await
    {
...
pub async fn handle_test_indexnow(
    ...
    let fallback = || {
        format!(
            "未检测到有效的密钥文件（{domain}/{key}.txt），若未采用文件验证方式可忽略此项检测结果"
        )
    };
        ...
        Ok(client) => match client.get(&url).send().await {
            Ok(response) => {
                ...
                match response.text().await {
                    Ok(body) if success && body.contains(key) => {
                        test_result(true, "密钥文件验证通过".to_string())
                    }
...
pub async fn handle_save_settings(
    ...
    let _ = state
        ...
        .save_site(
            ...
            form.bing_indexnow_key
                .as_deref()
                .filter(|s| !s.trim().is_empty()),
            form.bing_webmaster_api_key
                .as_deref()
                .filter(|s| !s.trim().is_empty()),
